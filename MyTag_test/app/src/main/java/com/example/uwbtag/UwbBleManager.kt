...
// 🎯 完美对应 MainActivity 的 5 大 UI 分支
enum class FindingStatus {
    IDLE,       // 等待点火
    SENDING,    // 正在点火...
    BOARD_STARTING, // board-only fallback 已发 OOB，等待板端启动诊断
    BOARD_STARTED,  // board-only fallback 下板端 FiRa responder 已启动
    RANGING,    // Android UWB ranging 已启动，等待手机侧结果
    SUCCESS,    // 🟢 链路测试成功（已收到 0x01 回声，UI会变绿并触发震动！）
    FAILED,     // 物理写入失败
    TIMEOUT,    // 板子未回传回声
    UWB_ERROR   // Relay B 已回传 UWB 失败诊断
}
...
class UwbBleManager(private val context: Context) {
    ...
    private val WRITE_CHARACTERISTIC_UUID = UUID.fromString("2E93998A-6A61-11ED-A1EB-0242AC120002")  // QNIS RX 写入通道
    private val NOTIFY_CHARACTERISTIC_UUID = UUID.fromString("2E939AF2-6A61-11ED-A1EB-0242AC120002") // QNIS TX Notify回传通道
    private val CLIENT_CHARACTERISTIC_CONFIG_UUID = UUID.fromString("00002902-0000-1000-8000-00805f9b34fb") // CCCD
    ...
    // 向 QNIS RX 写入 Android OOB START_SESSION 参数包。
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        // 🎯 核心反馈闭环点：在这里接收 Relay B 利用 ble_qnis_data_send(...) 发回的数据。
